...
#sleep 1
#/etc/init.d/usb_gadget_configfs.sh uvc 0 cdns3 0x1d6b 0x0102
...
if [ -e "/local/usr/bin/uvc_service" ]; then
	...
	#/tmp/uvc_service -cdns3 --mode uvc --$ceva0_isp_function --fps $camera_fps &
#	/tmp/uvc_service --mode hdmi --$ceva0_isp_function --fps $camera_fps &
fi
