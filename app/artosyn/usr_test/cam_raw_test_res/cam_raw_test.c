...
static ar_mm_handle_t get_multimedia_core_device_handle(void)
{
    static ar_mm_handle_t handle = NULL;

    if(handle) // have handle
        return handle;

    printf("multimedia_core_device_handle is null, so creat a new one..");
...

{
    ...
    handle = get_multimedia_core_device_handle();
...
static int set_cam_res(uint32_t w, uint32_t h)
{
    cam_raw_context_t* context = NULL;
    uint32_t context_size = 0;
    //get hanle and pipeline
    ar_mm_handle_t handle = get_multimedia_core_device_handle();
    cam_raw_get_context((void**)&context, &context_size);
    //stop
...
int stress_tst(uint32_t times)
{
    set_cam_res(1920, 1080);
    return 0;
}
...
void get_sink_buf_0_thread(void *arg)
{
    ...
    // while(obj->frame_count > 0){
        ...
        pthread_cond_signal(&obj->buf_signal);// 释放被条件阻塞的线程，条件重新复位
        pthread_mutex_unlock(&obj->mutex);
...
int main (int argc, char *argv[])
{
	...
	if(pra_index>0)
	{
        ...
        printf("--t_times: set res test times\n");
		return 0;
	}
    ...
    pra_index = find_pra_index_by_name("--t_times", argc, argv);
    if(pra_index > 0){
        test_cycles = atoi(argv[pra_index + 1]);
    ...
    while(1){
        ...
        pthread_cond_init(&obj.buf_signal,NULL);// 初始化条件变量
        ...
        printf("--------The %d times cam test over!---------\n", ts);
        end = get_current_time();
        printf("[set_res]cost time %lluus\r\n",end-start);
        //reset stream
        start = get_current_time();
        // if(ts%2)
            set_cam_res(1920, 1080);
        // else
            // set_cam_res(1920, 960); //在这里设置不同size后，rtos log报错
