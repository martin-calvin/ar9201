...
#define CAM_SRC_PAD_NUM_UVC    0   //cam_src接uvc的pad编号
#define CAM_SRC_PAD_NUM_NOISE  1   //cam_src接3D去噪的pad编号
...
void ar_uvc_init(ar_mm_handle_t handle,int argc, char *argv[])
{
    ...
    INFO("res0 w=%d h=%d\n", width, height);
    ar_pad_modify_meta_value(handle,ar_element_get_src_pad_by_idx(handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_WIDTH_NAME,&width,sizeof(int));
    ar_pad_modify_meta_value(handle,ar_element_get_src_pad_by_idx(handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_HEIGHT_NAME,&height,sizeof(int));

    width = server_l->width[1];
    height = server_l->height[1];
    log_always("res1 w=%d h=%d",width, height);
    ar_pad_modify_meta_value(handle,ar_element_get_src_pad_by_idx(handle,context->cam_src.element, CAM_SRC_PAD_NUM_NOISE),AR_COMMON_META_VIDEO_WIDTH_NAME,&width,sizeof(int));
    ar_pad_modify_meta_value(handle,ar_element_get_src_pad_by_idx(handle,context->cam_src.element, CAM_SRC_PAD_NUM_NOISE),AR_COMMON_META_VIDEO_HEIGHT_NAME,&height,sizeof(int));

    //set fps
    INFO("fps=%d\n", num);
    ar_pad_modify_meta_value(handle,ar_element_get_src_pad_by_idx(handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_FPS_NUM_NAME,&num,sizeof(int));
    ar_pad_modify_meta_value(handle,ar_element_get_src_pad_by_idx(handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_FPS_DEN_NAME,&den,sizeof(int));
    ar_pad_modify_meta_value(handle,ar_element_get_src_pad_by_idx(handle,context->cam_src.element, CAM_SRC_PAD_NUM_NOISE),AR_COMMON_META_VIDEO_FPS_NUM_NAME,&num,sizeof(int));
    ar_pad_modify_meta_value(handle,ar_element_get_src_pad_by_idx(handle,context->cam_src.element, CAM_SRC_PAD_NUM_NOISE),AR_COMMON_META_VIDEO_FPS_DEN_NAME,&den,sizeof(int));
    ...
    /* link pads */
    ar_pad_link_by_element(handle,context->cam_src.element,CAM_SRC_PAD_NUM_UVC,context->yuv_0_tee.element, 0);
    //ar_pad_link_by_element(handle,context->yuv_0_tee.element, 0,context->comp.element, 0);


    if(NULL == server_l->dev || V4L2_PIX_FMT_MJPEG == server_l->dev->fcc || 0 == server_l->dev->fcc)
        ar_pad_link_by_element(handle,context->yuv_0_tee.element, 0,context->mjpeg_enc.element, 0);
    else if(V4L2_PIX_FMT_HEVC == server_l->dev->fcc)
        ar_pad_link_by_element(handle,context->yuv_0_tee.element, 0,context->hevc_enc.element, 0);
    else if(V4L2_PIX_FMT_H264 == server_l->dev->fcc)
        ar_pad_link_by_element(handle,context->yuv_0_tee.element, 0,context->avc_enc.element, 0);
    else if(V4L2_PIX_FMT_NV12 == server_l->dev->fcc || V4L2_PIX_FMT_YUYV == server_l->dev->fcc || V4L2_PIX_FMT_YUV420 == server_l->dev->fcc)
        ar_pad_link_by_element(handle,context->yuv_0_tee.element, 0,context->yuv_enc.element, 0);

    
    ar_pad_link_by_element(handle,context->yuv_0_tee.element, 1,context->yuv_1_sink.element, 0);
    ar_pad_link_by_element(handle,context->cam_src.element, CAM_SRC_PAD_NUM_NOISE,context->yuv_0_sink.element, 0);
...
void ar_uvc_deinit(ar_mm_handle_t handle)
{
    ...
    ar_pad_unlink_by_element(handle,context->cam_src.element, CAM_SRC_PAD_NUM_NOISE,context->yuv_0_sink.element, 0);
    ar_pad_unlink_by_element(handle,context->yuv_0_tee.element, 1,context->yuv_1_sink.element, 0);

    if(NULL == server_l->dev || V4L2_PIX_FMT_MJPEG == server_l->dev->fcc || 0 == server_l->dev->fcc)
        ar_pad_unlink_by_element(handle,context->yuv_0_tee.element, 0,context->mjpeg_enc.element, 0);
    else if(V4L2_PIX_FMT_HEVC == server_l->dev->fcc)
        ar_pad_unlink_by_element(handle,context->yuv_0_tee.element, 0,context->hevc_enc.element, 0);
    else if(V4L2_PIX_FMT_H264 == server_l->dev->fcc)
        ar_pad_unlink_by_element(handle,context->yuv_0_tee.element, 0,context->avc_enc.element, 0);
    else if(V4L2_PIX_FMT_NV12 == server_l->dev->fcc || V4L2_PIX_FMT_YUYV == server_l->dev->fcc || V4L2_PIX_FMT_YUV420 == server_l->dev->fcc)
        ar_pad_unlink_by_element(handle,context->yuv_0_tee.element, 0,context->yuv_enc.element, 0);


    //ar_pad_unlink_by_element(handle,context->yuv_0_tee.element, 0,context->comp.element, 0);
    ar_pad_unlink_by_element(handle,context->cam_src.element,CAM_SRC_PAD_NUM_UVC,context->yuv_0_tee.element, 0);
...
int uvc_pipeline_set_format(unsigned int width, unsigned int height, unsigned int fps, unsigned int fcc)
{
    ...
    char *p = (char *)&fcc;
    INFO("change format to 0x%08x(%c%c%c%c) %ux%u@%ufps\n", fcc, p[0], p[1], p[2], p[3], width, height, fps);
    if(width < height)
    {
        ar_pad_modify_meta_value(server_l->handle,ar_element_get_src_pad_by_idx(server_l->handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_WIDTH_NAME,&height,sizeof(int));
        ar_pad_modify_meta_value(server_l->handle,ar_element_get_src_pad_by_idx(server_l->handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_HEIGHT_NAME,&width,sizeof(int));
    }
    else
    {
        ar_pad_modify_meta_value(server_l->handle,ar_element_get_src_pad_by_idx(server_l->handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_WIDTH_NAME,&width,sizeof(int));
        ar_pad_modify_meta_value(server_l->handle,ar_element_get_src_pad_by_idx(server_l->handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_HEIGHT_NAME,&height,sizeof(int));
    }
    ar_pad_modify_meta_value(server_l->handle,ar_element_get_src_pad_by_idx(server_l->handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_FPS_NUM_NAME,&fps,sizeof(int));
    ar_pad_modify_meta_value(server_l->handle,ar_element_get_src_pad_by_idx(server_l->handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_FPS_DEN_NAME,&den_num,sizeof(int));
    ar_pad_modify_meta_value(server_l->handle,ar_element_get_src_pad_by_idx(server_l->handle,context->cam_src.element, CAM_SRC_PAD_NUM_NOISE),AR_COMMON_META_VIDEO_FPS_NUM_NAME,&fps,sizeof(int));
    ar_pad_modify_meta_value(server_l->handle,ar_element_get_src_pad_by_idx(server_l->handle,context->cam_src.element, CAM_SRC_PAD_NUM_NOISE),AR_COMMON_META_VIDEO_FPS_DEN_NAME,&den_num,sizeof(int));
    if(V4L2_PIX_FMT_H264 == server_l->dev->fcc || V4L2_PIX_FMT_HEVC == server_l->dev->fcc)
...
static int pipeline_clt_set_prop_uvc(pipeline_ctl_server_t *server_l, char *arg, int len)
{
    ...
    if(index >=0)
    {
        ...
        int den = 1;
        INFO("set fps =%d",num);
        ar_pad_modify_meta_value(server_uvc->handle,ar_element_get_src_pad_by_idx(server_uvc->handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_FPS_NUM_NAME,&num,sizeof(int));
        ar_pad_modify_meta_value(server_uvc->handle,ar_element_get_src_pad_by_idx(server_uvc->handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_FPS_DEN_NAME,&den,sizeof(int));
        ar_pad_modify_meta_value(server_uvc->handle,ar_element_get_src_pad_by_idx(server_uvc->handle,context->cam_src.element, CAM_SRC_PAD_NUM_NOISE),AR_COMMON_META_VIDEO_FPS_NUM_NAME,&num,sizeof(int));
        ar_pad_modify_meta_value(server_uvc->handle,ar_element_get_src_pad_by_idx(server_uvc->handle,context->cam_src.element, CAM_SRC_PAD_NUM_NOISE),AR_COMMON_META_VIDEO_FPS_DEN_NAME,&den,sizeof(int));
    }
	...
	if(index >=0)
	{
		 ...
		 ar_pad_modify_meta_value(server_uvc->handle,ar_element_get_src_pad_by_idx(server_uvc->handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_WIDTH_NAME,&w,sizeof(int));
		 ar_pad_modify_meta_value(server_uvc->handle,ar_element_get_src_pad_by_idx(server_uvc->handle,context->cam_src.element, CAM_SRC_PAD_NUM_UVC),AR_COMMON_META_VIDEO_HEIGHT_NAME,&h,sizeof(int));
	}
