...
typedef struct _D2CDist
{
	uint16_t keyValue;
	uint16_t depth;
} D2CDist;

typedef struct _D2C_PARAMS
{
	bool depth_undist_switch; //深度图是否去畸变
	bool rgb_undist_switch;	  //rgb是否去畸变
	bool isDepthMirror;		  //深度是否镜像
	float depth_unit_mm;	  //深度单位0.1mm 或者1mm
	bool enable_gap_fill;	  //是否填充
	bool enable_depth_Copy4;  //开启1copy4填补空洞策略
	float depth_min_value;	  //深度范围最小值
...
typedef struct _DepthToColorImpl
{
	...
	float color_K_[4];			// fx fy cx cy;
	float rot_[9];				// rot matrix
