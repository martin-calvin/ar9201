 ...
 static void *tof_uvc_gadget_events_poll_thd(void *arg)
 {
     ...
     while (1 == pService->pollrun)    
     {
         ...
         if (ret > 0)
         {
             ...
             {
                 ob_tof_uvc_events_process(pService->dev);
             }
         }
     }
...
*   Return Value : void t
*****************************************************************************/
 void tof_uvc_gadget_register_cmd_cb(int dev_index, ob_tof_uvc_gadget_cmd_cb cb)
 {
     tof_uvc_service *pService = tof_uvc_gadget_get_service(dev_index);
     if (!pService){
         ERR("pObdev is NULL.\n");
         return;
     } 
 ...
 *****************************************************************************/
 tof_uvc_service *tof_uvc_gadget_get_service(int dev_index)
 {
 ...
 *****************************************************************************/
 int32_t tof_uvc_gadget_init(int dev_index)
 {
     ...
     pService = tof_uvc_gadget_get_service(dev_index);
     if (!pService)
     {
         ERR("pService is NULL.\n");
         goto FAIL_EXIT;
     ...
     pthread_mutex_init(&pService->g_stream_control_mutex, NULL); 
