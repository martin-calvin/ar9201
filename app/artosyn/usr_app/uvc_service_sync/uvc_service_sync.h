#ifndef __UVC_SERVICE_SYNC_H__
#define __UVC_SERVICE_SYNC_H__
...
typedef struct uvc_server uvc_server_t;
typedef struct uvc_format_data  uvc_format_data_t;

struct uvc_format_opt
{
    int (*format_init)(uvc_server_t * server_l, uvc_format_data_t *data, ar_mm_handle_t handle, int argc, char *argv[]);
    int (*format_deinit)(uvc_server_t * server_l, uvc_format_data_t *data, ar_mm_handle_t handle);
    struct uvc_frame_buf_info * (*get_frame)(uvc_server_t * server_l, uvc_format_data_t *data);
    int (*release_frame)(uvc_server_t * server_l, uvc_format_data_t *data, struct uvc_frame_buf_info * buf_info);
};
...
typedef enum
{
    ...
    UVC_STATUS_STOP,            //only master
    UVC_STATUS_CHANGE_FORMAT,   //only master
    UVC_STATUS_START,           //only master
    UVC_STATUS_PAUSE,           //only slave
    UVC_STATUS_EXIT,
    UVC_STATUS_MAX
}uvc_status;
...
typedef struct uvc_server
{
    struct uvc_server     *master;
    struct uvc_device     *dev;
    struct uvc_server    **slave;
    int                    slave_num;
    pthread_mutex_t        g_wrt_com_mutex;
    pthread_mutex_t       *p_chg_fmt_mutex;
    chg_fmt_status         g_chg_fmt_flag;
    struct uvc_server     *chg_fmt_server;
...
extern struct uvc_format_opt uvc_h265_opt;
extern struct uvc_format_opt uvc_h264_opt;
extern struct uvc_format_opt uvc_mjpeg_opt;
extern struct uvc_format_opt uvc_nv12_opt;
extern struct uvc_format_opt uvc_yuy2_opt;
