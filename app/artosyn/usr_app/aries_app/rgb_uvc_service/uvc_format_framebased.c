...
static int uvc_h264_deinit(uvc_format_data_t *data, ar_mm_handle_t handle);


//h264 h265走同一个init api
static int uvc_h264_init(uvc_format_data_t *data, ar_mm_handle_t handle, int argc, char *argv[])
{
    ...
    if(NULL == priv_data)
    {
        ERR("framebased_priv_data malloc failed!\n");
        goto H264_INIT_FAILED;
    }
...
static struct uvc_frame_buf_info * uvc_h264_get_frame(uvc_format_data_t *data)
{
...
static int uvc_h264_release_frame(uvc_format_data_t *data, struct uvc_frame_buf_info * buf_info)
{
    int ret = -1;
...
static struct uvc_frame_buf_info * uvc_h265_get_frame(uvc_format_data_t *data)
{
    struct uvc_frame_buf_info       *buf_info           = NULL;
    ar_video_stream_buffer_t        *stream_buf_info    = NULL;
    struct framebased_priv_data     *priv_data          = NULL;
