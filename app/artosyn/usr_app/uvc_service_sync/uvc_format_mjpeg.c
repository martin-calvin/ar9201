...
static int uvc_mjpeg_deinit(uvc_server_t * server_l, uvc_format_data_t *data, ar_mm_handle_t handle);

static int uvc_mjpeg_init(uvc_server_t * server_l, uvc_format_data_t *data, ar_mm_handle_t handle, int argc, char *argv[])
{
...
MJPEG_INIT_FAILED:
    uvc_mjpeg_deinit(server_l, data, handle);
...
static struct uvc_frame_buf_info * uvc_mjpeg_get_frame(uvc_server_t * server_l, uvc_format_data_t *data)
{
...
static int uvc_mjpeg_release_frame(uvc_server_t * server_l, uvc_format_data_t *data, struct uvc_frame_buf_info * buf_info)
{
    int ret = -1;
...
struct uvc_format_opt uvc_mjpeg_opt =
{
