...
typedef struct tof_uvc_device
{
    ...
    USB_VERSION_EM          emVerUSB;      //usb specification version
    int                     fd;            //uvc fd:    "/dev/videoN"
    int                     is_streaming;  //uvc stream on flag
    ...
    int                     tof_stream_type;//refer to: TOF_UVC_RQ_STREAM_TYPE
}tof_uvc_device;


//for depth/rawPhase and ir
//tof_uvc_gadget_get_service
typedef struct _tof_uvc_service 
{
    struct tof_uvc_device *dev;
    int                    dev_index;      // 1: depth/RawPhase 2:ir  refer to: OB_UVC_DEVICE_NODE_NUM_DEPTH

    int                    run_flag;        //uvc service run flag

    pthread_mutex_t        g_wrt_com_mutex; //uvc数据发送buff占用互斥锁
    pthread_mutex_t        g_stream_control_mutex;
    ...
    ob_tof_uvc_gadget_cmd_cb *cmd_cb;       //==> ob_tof_usb_gadget_cmd_from_host
}tof_uvc_service;
...
*   Return Value : int
*****************************************************************************/
extern int ob_tof_uvc_video_set_format(struct tof_uvc_device *dev);
...
*   Return Value : void
*****************************************************************************/
extern void ob_tof_uvc_events_process(struct tof_uvc_device *dev);
