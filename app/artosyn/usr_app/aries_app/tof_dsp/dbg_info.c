...
void debug_info_tofparams_msg(ObTofDspParams *tof)
{
    ...
    printf("tof->inputs.constInputs.calibParams.hdr.Type = %d\r\n", tof->inputs.constInputs.calibParams.hdr.Type);

    printf("tof->inputs.constInputs.calibParams.hdr.identity = ");
    for(int i =0; i< 8; i++){
        printf("%d ", tof->inputs.constInputs.calibParams.hdr.identity[i]);
    }
    printf("\r\n");

    printf("tof->inputs.constInputs.calibParams.dev_info.dev_sn = ");
    for(int i =0; i< 32; i++){
        printf("%d ", tof->inputs.constInputs.calibParams.dev_info.dev_sn[i]);
    }
    printf("\r\n");

    printf("tof->inputs.constInputs.calibParams.dev_info.ir_sn = ");
    for(int i =0; i< 32; i++){
        printf("%d ", tof->inputs.constInputs.calibParams.dev_info.ir_sn[i]);
    }
    printf("\r\n");

    printf("tof->inputs.constInputs.calibParams.dev_info.vcsel_sn = ");
    for(int i =0; i< 32; i++){
        printf("%d ", tof->inputs.constInputs.calibParams.dev_info.vcsel_sn[i]);
    }
    printf("\r\n");
    
    // printf("tof->inputs.constInputs.calibParams.dev_info.dev_sn = %s\r\n", tof->inputs.constInputs.calibParams.dev_info.dev_sn);
    // printf("tof->inputs.constInputs.calibParams.dev_info.dev_sn = %s\r\n", tof->inputs.constInputs.calibParams.dev_info.ir_sn);
    // printf("tof->inputs.constInputs.calibParams.dev_info.dev_sn = %s\r\n", tof->inputs.constInputs.calibParams.dev_info.vcsel_sn);
    ...
    printf("tof->inputs.constInputs.calibParams.dist_slop_calib.fppn_offset = %f\r\n", tof->inputs.constInputs.calibParams.dist_calib[0].fppn_offset);
    printf("tof->inputs.constInputs.calibParams.dist_slop_calib.fppn_slop_lut_0 = 0x%x\r\n", tof->inputs.constInputs.calibParams.dist_calib[0].fppn_slope_lut);
    printf("tof->inputs.constInputs.calibParams.dist_slop_calib.fppn_slop_lut_1 = 0x%x\r\n", tof->inputs.constInputs.calibParams.dist_calib[1].fppn_slope_lut);

    printf("tof->inputs.constInputs.calibParams.freq_mod[0] = %d\r\n", tof->inputs.constInputs.calibParams.sensor_param[0].freq_mod);
    printf("tof->inputs.constInputs.calibParams.freq_mod[1] = %d\r\n", tof->inputs.constInputs.calibParams.sensor_param[1].freq_mod);

    printf("tof->inputs.constInputs.calibParams.temp_comp[0].sensor_temp_comp_coeff = %p\r\n", tof->inputs.constInputs.calibParams.temp_comp[0].sensor_temp_comp_coeff);
    ...
    printf("tof->inputs.constInputs.calibParams.temp_comp[0].vcsel_temp_comp_coeff = %p\r\n", tof->inputs.constInputs.calibParams.temp_comp[0].vcsel_temp_comp_coeff);
    printf("tof->inputs.constInputs.calibParams.temp_comp[0].sensor_coeff_size = %d\r\n", tof->inputs.constInputs.calibParams.temp_comp[0].sensor_coeff_size);
    printf("tof->inputs.constInputs.calibParams.temp_comp[0].vcsel_coeff_size = %d\r\n", tof->inputs.constInputs.calibParams.temp_comp[0].vcsel_coeff_size);

    printf("tof->inputs.constInputs.calibParams.temp_comp[1].sensor_temp_comp_coeff = %p\r\n", tof->inputs.constInputs.calibParams.temp_comp[1].sensor_temp_comp_coeff);
    ...
    printf("tof->inputs.constInputs.calibParams.temp_comp[1].vcsel_temp_comp_coeff = %p\r\n", tof->inputs.constInputs.calibParams.temp_comp[1].vcsel_temp_comp_coeff);
    printf("tof->inputs.constInputs.calibParams.temp_comp[1].sensor_coeff_size = %d\r\n", tof->inputs.constInputs.calibParams.temp_comp[1].sensor_coeff_size);
    printf("tof->inputs.constInputs.calibParams.temp_comp[1].vcsel_coeff_size = %d\r\n", tof->inputs.constInputs.calibParams.temp_comp[1].vcsel_coeff_size);


    printf("tof->inputs.constInputs.cameraIntrin.cx = %f\r\n", tof->inputs.constInputs.calibParams.ir_intrin.cx);
    printf("tof->inputs.constInputs.cameraIntrin.cy = %f\r\n", tof->inputs.constInputs.calibParams.ir_intrin.cy);
    printf("tof->inputs.constInputs.cameraIntrin.fx = %f\r\n", tof->inputs.constInputs.calibParams.ir_intrin.fx);
    printf("tof->inputs.constInputs.cameraIntrin.fy = %f\r\n", tof->inputs.constInputs.calibParams.ir_intrin.fy);
#endif
